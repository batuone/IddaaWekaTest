...
namespace IddaaWekaTest
{
    class EvSahibiTahminWekaServisNew
    {
        ...
        public String calistirTahmin(bool tableInsert, string[] ligler)
        {
            ...
            using (var ctx = new IDDAA_Entities())
            {
                ...
                foreach (var lig in gununLigleri)
                {
                    ...
                    //    sabitDeger.evSahibiSonuc);
                    lstOgrenmeButunAttributelar = macSonuOgrenmeServisNew.ekleMacSonuOgrenmeButunAttributeler
                        (ligPair, dahilOgrenme.macSonuDahilAttribute, sabitDeger.evSahibiSonuc);
                    lstOgrenme = macSonuOgrenmeServisNew.convertOgrenmeClassToOgrenmeContext(lstOgrenmeButunAttributelar, dahilOgrenme.macSonuDahilAttribute, sabitDeger.evSahibiSonuc);
                ...
                if(tableInsert)
                {
                    tahminMLServis.ekleTahminMLList(lstSonucGenel, ligler, sabitDeger.macSonuSonuc, "1");
        ...
        private List<Sonuc> calistirTahminSonuc(string[] kullanilacakAttribute, string[] ligPair, List<OgrenmeClass> lstOgrenmeButunAttributelar)
        {
            ...
            //lstOgrenme = macSonuOgrenmeServisNew.ekleMacSonuOgrenmeFull(ligPair, kullanilacakAttribute, sabitDeger.evSahibiSonuc);
            lstOgrenme = macSonuOgrenmeServisNew.convertOgrenmeClassToOgrenmeContext(lstOgrenmeButunAttributelar, kullanilacakAttribute, sabitDeger.evSahibiSonuc);
            ...
            double evSahibiOran;
            double macProbability = helper.macProbability(sabitDeger.evSahibiSonuc, ligPair);
            foreach (var mac in linesTestOgrenme)
            {
                ...
                lstSonucHepsi.Add(ekleSonuc(mac, evSahibiProb, "E"));

                answer = evSahibiProb > macProbability ? "E" : "D";
                ...
                using (var ctx = new IDDAA_Entities())
                {
                    ...
                    if (evSahibiProb < macProbability)
                    {
                        continue;
                    }
                    evSahibiOran = Convert.ToDouble(ctx.BULTEN.First(c => c.IDDAA_ID == mac.IDDAA_ID).MS_1);
                    if (evSahibiOran < sabitDeger.macSonuTahminMinBahisIddaaOran)
                    {
                        continue;
                    }
                }

                lstSonuc.Add(ekleSonuc(mac, evSahibiProb, answer));
            }
            ...
            string[] ligler)
        {
            ...
            tahminTestServis.ekleTahminTestList(ligler, sabitDeger.macSonuSonuc, "1", karMapParallel.First().Value, karMapParallel.First().Key,
                karTestParallel.testYuzdeOran);


            if (karMapParallel.First().Value <= sabitDeger.evSahibiKarTutar)
            {
                return null;
            }

            helper.sendTelegramKarTutar(ligler, karMapParallel.First().Value, sabitDeger.evSahibiSonuc, karMapParallel.First().Key);
